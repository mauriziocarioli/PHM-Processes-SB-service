...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "PHM-Processes-SB-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name PHM-Processes-SB-service apps/PHM-Processes-SB-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app PHM-Processes-SB-service:1.0-SNAPSHOT
		call oc expose svc/PHM-Processes-SB-service
	) else (
